...
# Description: This file executes the entire experiment not using a Continual Learning method,
# but simply stacking the data of all the previous experiences as training data for the current one.
...
def cumulative_train_eval_model(
        model, train_dict, val_dict, test_dict, criterion, optimizer,
        experiences_targets, batch_size, n_epochs,
        smoothing_window, early_stop_patience=np.inf,
        early_stop_margin=0, eval_every=1,
        device="cpu", growing_model=False):

    experiences_targets = [
        [item for sublist in experiences_targets[:i + 1] for item in sublist]
        for i in range(len(experiences_targets))
    ]
    ...
    for current_experience in range(n_experiences):
        ...
        optimizer = optim.Adam(model.parameters(), lr=optimizer.param_groups[-1]['lr'])
