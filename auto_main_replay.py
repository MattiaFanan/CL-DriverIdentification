...
# Description: This file executes the hyperparameter search for the Experience Replay method.
...
if __name__ == '__main__':
    ...
    memory_mixin = [0.25, 0.5, 0.75]
    memory_sizes = [100, 500, 750, 1000, 1500, 2000, 4000]
    ...
    for xexp in xexp_list_split:
        data_orders_runs = [random_runs_order(original_classes, xexp) for _ in range(num_different_class_orders)]
        for memory_size in memory_sizes:
            for mixin in memory_mixin:
                for i, experiences_targets in enumerate(data_orders_runs):
                    for seed in seeds:
                        run_name = f"{xexp}_xexp_split_{memory_size}_size_{mixin}_mixin_{seed}_seed_{i}_order_id"
                        main_args = base_main_args + [
                            "--run_name", run_name,
                            "--seed", str(seed),
                            "--experiences_targets", str(experiences_targets),
