...
# Description: This file executes the hyperparameter search for the EWC algorithm.
...
if __name__ == '__main__':
    ...
    ewc_lambdas = [1, 50, 100, 500, 1000, 5000, 10000]
    ...
    for xexp in xexp_list:
        ...
        for ewc_lambda in ewc_lambdas:
            for i, experiences_targets in enumerate(data_orders):
                for seed in seeds:
                    run_name = f"{xexp}_xexp_{ewc_lambda}_EWClambda_{seed}_seed_{i}_order_id"
    ...
    for xexp in xexp_list_split:
        ...
        for ewc_lambda in ewc_lambdas:
            for i, experiences_targets in enumerate(data_orders_runs):
                for seed in seeds:
                    run_name = f"{xexp}_xexp_split_{ewc_lambda}_EWClambda_{seed}_seed_{i}_order_id"
                    main_args = base_main_args + [
                        "--run_name", run_name,
                        "--seed", str(seed),
                        "--experiences_targets", str(experiences_targets),
