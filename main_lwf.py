...
# using the Learning Without Forgetting (LwF) strategy.
...
def lwf_train_eval_model(
        model, train_dict, val_dict, test_dict, criterion, optimizer,
        experiences_targets, batch_size, lwf_lambda,
        ...
        for epoch in tqdm(range(n_epochs), unit="epoch", total=n_epochs):
            ...
            for train_x, train_y in experience_train_dataloader:
                ...
                if old_model is not None:
                    # prevent backpropagation through old_model that is in eval mode
                    with torch.no_grad():
                        outputs_target = old_model(train_x)
                    old_dimensions = outputs_target.shape[1]
                    # we need to explicitly slice it here otherwise it is not added to the computational graph
                    sliced_train_outputs = train_outputs[:, :old_dimensions]
                    loss_lwf = lwf_lambda * kl_divergence(sliced_train_outputs, outputs_target)
                    wandb.log({"Train/Loss LWF": loss_lwf})
                    loss += loss_lwf
                loss.backward()
                optimizer.step()
                with torch.no_grad():
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lwf_lambda', type=float, required=True, help='Lambda for LWF')
    ...
    lwf_train_eval_model(
        ...
        lwf_lambda=wandb.config["lwf_lambda"],
