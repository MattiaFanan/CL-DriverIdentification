...
# Description: This file is a copy of auto_ml_replay.py just to get the info on the model
...
def replay_train_eval_model(
        model, train_dict, val_dict, test_dict, criterion, optimizer,
        experiences_targets, memory_mixin, memory_size, batch_size,
    ...
    replay_memory = ReplayMemory(mem_size=memory_size)
    ...
    for current_experience in range(n_experiences):
        ...
        if not replay_memory.is_empty:
            batch_size_memory = int(batch_size * memory_mixin)
            batch_size_dataset = batch_size - batch_size_memory
            if batch_size_memory == 0:
                raise ValueError("Memory mixin is too small for the current batch size")
            if batch_size_dataset == 0:
                raise ValueError("Memory mixin is too high for the current batch size")
        else:
            batch_size_memory = 0
            batch_size_dataset = batch_size

        experience_train_dataloader = DataLoader(
            TensorDataset(train_exp_x, train_exp_y),
            batch_size=batch_size_dataset, shuffle=True, drop_last=True)
        exp_time += time.perf_counter() - start_time
        ...
        for epoch in tqdm(range(n_epochs), unit="epoch", total=n_epochs):
            ...
            for train_x, train_y in experience_train_dataloader:
                ...
                x_memory, y_memory = replay_memory.sample(batch_size_memory)
                x_memory = torch.tensor(x_memory).to(device).float()
                y_memory = torch.tensor(y_memory).to(device).long()
                train_x = torch.cat((train_x, x_memory))
                train_y = torch.cat((train_y, y_memory))
        ...
        start_time = time.perf_counter()
        replay_memory.add_to_memory(train_exp_x.detach().cpu().numpy(), train_exp_y.detach().cpu().numpy())
        exp_time += time.perf_counter() - start_time
...
if __name__ == '__main__':
    ...
    parser.add_argument('--memory_mixin', type=float, default=0.5, help='Memory mixin.')
    ...
    parser.add_argument('--wandb_folder', type=str, required=False,
                        help='folder to save the data. ./wandb/ if not specified.')
    ...
    config = {
        ...
        "memory_mixin": args.memory_mixin,
    ...
    if "wandb_folder" in args and args.wandb_folder is not None:
        wandb.init(project=args.project, config=config, entity=args.entity, name=args.run_name, dir=args.wandb_folder)
    else:
        wandb.init(project=args.project, config=config, entity=args.entity, name=args.run_name)
    ...
    # train the model
    final_model = replay_train_eval_model(
        model=model,
        ...
        memory_mixin=wandb.config["memory_mixin"],
    ...
    summary_info(
        final_model,
        input_size=(wandb.config["batch_size"], wandb.config["window_size"], num_features),
        batch_dim=None,
        device="cpu"
    )
    wandb.finish()
