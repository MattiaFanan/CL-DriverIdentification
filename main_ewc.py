...
# using the Elastic Weight Consolidation (EWC) strategy.
...
def ewc_train_eval_model(
        model, train_dict, val_dict, test_dict, criterion, optimizer,
        experiences_targets, batch_size, ewc_lambda, ewc_empirical,
    ...
    pred_accumulator = []
    ewc = EWC(ewc_lambda=ewc_lambda, empirical=ewc_empirical)
    ...
    for current_experience in range(n_experiences):
        ...
        start_time = time.perf_counter()
        # resize the model to the new number of classes
        ...
        for epoch in tqdm(range(n_epochs), unit="epoch", total=n_epochs):
            ...
            for train_x, train_y in experience_train_dataloader:
                ...
                # kl divergence if not first experience
                ewc_loss = ewc.get_ewc_loss(model)
                loss += ewc_loss
                loss.backward()
                optimizer.step()
                wandb.log({"Train/EWC": ewc_loss})
                with torch.no_grad():
        ...
        # update the importance before setting the model to evaluation mode
        start_time = time.perf_counter()
        ewc.update_importance(model, experience_train_dataloader, criterion)
        exp_time += time.perf_counter() - start_time
...
def kl_divergence(y, y_target):
    p_y = nn.functional.log_softmax(y, dim=-1)
    p_y_target = nn.functional.log_softmax(y_target, dim=-1)
    return nn.KLDivLoss(reduction='batchmean', log_target=True)(p_y, p_y_target)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ewc_lambda', type=float, required=True, help='Lambda for LWF')
    parser.add_argument('--ewc_empirical', action='store_true', default=False, help='Use empirical fisher')
    ...
    config = {
        ...
        "ewc_empirical": args.ewc_empirical,
    ...
    ewc_train_eval_model(
        ...
        ewc_lambda=wandb.config["ewc_lambda"],
        ewc_empirical=wandb.config["ewc_empirical"],
